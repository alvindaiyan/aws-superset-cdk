...
export interface SupersetCeleryParam {
...
export class SupersetCelery {
  ...
  constructor(scope: Construct, params: SupersetCeleryParam) {
    this.scope = scope;
    ...
    this.taskRole = this.supersetCeleryTaskRole();
    this.executionRole = this.supersetCeleryExecutionRole();
    this.serviceEntry = this.discoveryEntry();
    this.taskDef = this.supersetCeleryTaskDef();

    const initContainer = this.taskDef.addContainer('superset-node-init-container', this.initContainerDef());
    const supersetCeleryContainer = this.taskDef.addContainer('superset-node-container', this.supersetCeleryContainerDef());
    supersetCeleryContainer.addMountPoints({
      readOnly: false,
      containerPath: '/app/superset_home',
      sourceVolume: 'superset_home',
    });
    supersetCeleryContainer.addContainerDependencies({
  ...
  private supersetCeleryTaskDef(): TaskDefinition {
    return new FargateTaskDefinition(this.scope, 'superset-celery-taskdef', {
      cpu: 4096,
      memoryLimitMiB: 16384,
      taskRole: this.taskRole,
      executionRole: this.executionRole,
      family: `${this.clusterName}-superset-worker-beat`,
      runtimePlatform: {
  ...
  private supersetCeleryContainerDef(): ContainerDefinitionOptions {
    return {
      ...
      containerName: 'superset-worker-beat',
      user: 'root',
    };
  }

  private supersetCeleryService(): IService {
    const supersetService = new FargateService(this.scope, 'superset-celery-svc', <FargateServiceProps>{
    ...
    Tags.of(supersetService).add('com.docker.compose.service', 'superset-worker-beat');
  ...
  private discoveryEntry(): Service {
    return new aws_servicediscovery.Service(this.scope, 'superset-celery-discovery-entry', <ServiceProps>{
      description: 'Entry for "superset-worker-beat" service discovery in AWS Cloud Map.',
  ...
  private supersetCeleryExecutionRole(): aws_iam.IRole {
    ...
    const dbExecutionRole = new aws_iam.Role(this.scope, 'superset-celery-execrole', {
      assumedBy: principal,
    });
    ...
    Tags.of(dbExecutionRole).add('com.docker.compose.service', 'superset-worker-beat');
    return dbExecutionRole;
  }

  private supersetCeleryTaskRole(): IRole {
    const principal = new aws_iam.ServicePrincipal('ecs-tasks.amazonaws.com');
    principal.addToAssumeRolePolicy(new PolicyDocument(
      {
        statements: [
    ...
    const svcTaskRole = new aws_iam.Role(this.scope, 'superset-celery-taskrole', {
      assumedBy: principal,
    });
    ...
    Tags.of(svcTaskRole).add('com.docker.compose.service', 'superset-worker-beat');
    return svcTaskRole;
  }
}
